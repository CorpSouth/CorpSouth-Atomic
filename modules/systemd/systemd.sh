...
SYSTEM_PRESETS="${CONFIG_DIRECTORY}/systemd/system-preset"
SYSTEM_PRESETS_DIR="/usr/lib/systemd/system-preset"
SYSTEM_UNIT_DIR="/usr/lib/systemd/system"
USER_UNIT_DIR="/usr/lib/systemd/user"
USER_PRESETS="${CONFIG_DIRECTORY}/systemd/user-preset"
USER_PRESETS_DIR="/usr/lib/systemd/user-preset"
...
if [[ -d "${SYSTEM_PRESETS}" ]]; then
  if [[ -n $(find "${SYSTEM_PRESETS}" -type f) ]]; then
    echo "Copying 'system presets' to systemd system-preset directory"
    cp -r "${SYSTEM_PRESETS}"/* "${SYSTEM_PRESETS_DIR}"
  fi
fi
...
if [[ -d "${USER_PRESETS}" ]]; then
  if [[ -n $(find "${USER_PRESETS}" -type f) ]]; then
    echo "Copying 'user presets' to systemd user-preset directory"
    cp -r "${USER_PRESETS}"/* "${USER_PRESETS_DIR}"
  fi
fi
